...
export const handleBlockCollision = (
  ...
  const overlapX =
    ball.radius + block.width / 2 - Math.abs(ballCenterX - blockCenterX);
  const overlapY =
    ball.radius + block.height / 2 - Math.abs(ballCenterY - blockCenterY);
  ...
  blocks.forEach((block) => {
    ...
    if (block.special) {
      ...
      switch (block.special) {
        ...
        case "circle":
          ctx.beginPath();
          ctx.arc(centerX, centerY, block.width / 6, 0, Math.PI * 2);
          ctx.stroke();
          break;
        case "plus":
          ctx.beginPath();
